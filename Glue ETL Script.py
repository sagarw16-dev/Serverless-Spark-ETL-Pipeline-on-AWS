...
print(f"Writing daily review counts to {s3_analytics_path}daily_review_counts/ ...")
daily_frame = DynamicFrame.fromDF(df_daily_reviews.repartition(1), glueContext, "daily_reviews_df")
glueContext.write_dynamic_frame.from_options(
    frame=daily_frame,
    connection_type="s3",
...
# This query identifies "power users" — customers who have submitted the most reviews.
...
print(f"Writing top 5 customers to {s3_analytics_path}top_5_customers/ ...")
...
rating_frame = DynamicFrame.fromDF(df_rating_dist.repartition(1), glueContext, "rating_dist_df")
glueContext.write_dynamic_frame.from_options(
    frame=rating_frame,
    connection_type="s3",
